...
path_BurgerKing = create_folder('12_BurgerKing', folder) + '/12_BurgerKing_items.csv'
...
class BurgerKing:
    def __init__(self):
        ...
        self.base_url = 'https://czqk28jt.apicdn.sanity.io/v2023-08-01/graphql/prod_bk_gb/gen3'

    def scrape(self):
        response = requests.post(self.base_url,params=self.params,headers=self.headers,json=self.json_data)
        json_response = json.loads(response.text)
        all_categories = json_response['data']['StaticPage']['widgets'][0]['menu']['options']
        for all_category in all_categories:
            category_name = all_category['name']['locale']
            print(category_name)
            all_products = all_category['options']
            for all_product in all_products:
                try:
                    product_name = all_product['name']['locale']
                except:
                    product_name = ''
                try:
                    if len(all_product) == 9:
                        nutrition = all_product['options'][0]['option']['nutritionWithModifiers']
                    else:
                        nutrition = all_product['nutritionWithModifiers']
                except:
                    try:
                        nutrition = all_product['options'][0]['option']['mainItem']['nutritionWithModifiers']
                    except:

                        nutrition = ''
                if nutrition:
                    nutrition = {k : v for k,v in nutrition.items() if v != None and isinstance(v,str) != True}
                try:
                    if len(all_product) == 9:
                        allergens = all_product['options'][0]['option']['allergens']
                    else:
                        allergens = all_product['allergens']
                except:
                    allergens = ''
                if allergens:
                    allergens = {k : v for k,v in allergens.items() if isinstance(v,str) != True and v != None and v > 0}
                ...
                if os.path.exists(path_BurgerKing):
                    df.to_csv(path_BurgerKing, header=False, index=False, mode='a')
                else:
                    df.to_csv(path_BurgerKing, header=True, index=False, mode='a')

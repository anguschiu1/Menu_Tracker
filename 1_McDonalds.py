...
def parse_combo(dat):
    ...
    with open(fileName, 'w') as f:
        json.dump(dat, f)
    item_type = dat.get('meal_item', {}).get('item_type', [])
    components = dat.get('meal_item', {}).get('items', [])
    components_list = []
    if components:
        for component in components.get('item', []):
            component_id = component.get('id')
            url_component = f'https://www.mcdonalds.com/dnaapp/itemDetails?country=UK&language=en&showLiveData=true&item={component_id}'
    ...
    if components_list:
        ...
        dat_dict = {
            'Product_Name': dat.get('meal_item').get('item_name'),
            'Product_Title': dat.get('meal_item').get('item_meta_title'),
            'Product_Description': ', '.join([component.get('Product_Name') for component in components_list]),
            'Product_Ingredients': cleanhtml(dat.get('meal_item').get('item_ingredient_statement')),
            'Product_Category': dat.get('meal_item').get('default_category').get('category').get('name') if dat.get('meal_item').get(
                'default_category') else 'NA',
            'item_type': item_type
        }
...
for item_id in item_ids:
    ...
    # print('-------------------------')
    # i+=1
    url = f'https://www.mcdonalds.com/dnaapp/itemDetails?country=UK&language=en&showLiveData=true&item={item_id}'
    # url = 'https://www.mcdonalds.com/dnaapp/itemCollectionDetails?country=UK&language=en&showLiveData=true&item=200447&specialCall=true'
    # url = f'https://www.mcdonalds.com/wws/json/getItemDetails.htm?country=UK&language=en&showLiveData=true&item={item_id}'
    dat = requests.get(url).json()
    if 'error' in dat.keys():
        pass
    else:
        ...
        try:
            ...
            url = f'https://www.mcdonalds.com/dnaapp/itemCollectionDetails?country=UK&language=en&showLiveData=true&item={item_id}&specialCall=true'
            dat = requests.get(url).json()

...
class A36BillsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        cat_links = response.xpath("//div[contains(concat(' ', normalize-space(@class), ' '), ' k10-menu-selector__option ')]//@data-menu-identifier").getall()
        cat_links = set(cat_links)
        print(f'No. of category links: {len(cat_links)}')

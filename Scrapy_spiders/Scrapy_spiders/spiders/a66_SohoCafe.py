...
class A66SohocafeSpider(scrapy.Spider):
    ...
    def parse_item(self, response): 
        ...
        item_dict = {
            ...
            'item_description': response.xpath('//section[contains(@class,"wp-block-uagb-section")]/div/p/text()').get()}
        for row in table: 
            ...
            if len(values) == 1:
                ...
                item_dict.update({
                 ...
                 item_dict.update({
                    header+'_100g': values[0]+' / '+values[1],
                    header + '_perserving': values[2]+' / '+values[3],
                })
        yield item_dict
